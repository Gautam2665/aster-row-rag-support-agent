...
def test_custom_cases_loading():
    ...
    assert "custom-incomplete-order-id-clarification-followup" in case_ids


def test_custom_cases_execution():
    """Test running all 6 custom evaluation cases through EvaluationRunner."""
    custom_path = Path("evaluation/custom-cases.json")
    runner = EvaluationRunner(cases_json_path=custom_path)
...
def test_clarification_followup_case_behavior():
    """Test custom-incomplete-order-id-clarification-followup case specifically."""
    runner = EvaluationRunner(cases_json_path=Path("evaluation/custom-cases.json"))
    cases = runner.load_cases()
    case_dict = next(c for c in cases if c["id"] == "custom-incomplete-order-id-clarification-followup")
    report = runner.evaluate_case(case_dict)
...
def test_combined_cases_execution():
    ...
    assert len(reports) == 21  # 15 visible + 6 custom
    assert all(r.deterministic_status == "PASS" for r in reports)
